...
export default function ViewJobsPosted() 
{
  ...
  const fetchJobs = async () => {
    try {
      const response = await axios.get(`http://localhost:2014/viewjobsbyjobseeker/`);
  ...
  const applyJob = async (jobid, jobseekeremail) => {
    try 
    {
      const response = await axios.post('http://localhost:2014/applyjob', { jobid, jobseekeremail });
      fetchJobs();
