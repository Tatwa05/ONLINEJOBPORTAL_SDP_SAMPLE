  ...
  const handleSubmit = async (e) => 
  {
    ...
    try 
    {
      ...
      if (Object.keys(updatedData).length !== 0) {
        ...
        const response = await axios.put('http://localhost:2014/updatejobseekerprofile', updatedData);
        setMessage(response.data);
        setError('');
        const res = await axios.get(`http://localhost:2014/jobseekerprofile/${jobseekerData.email}`, updatedData)
