...
export default function ViewJobApplicants() {
  ...
  const fetchJobApplicants = async () => {
    try 
    {
      const response = await axios.get(`http://localhost:2014/viewjobapplicants/${recruiterData.username}`);
  ...
  const handleStatusChange = async (applicantId, status) => {
    try 
    {
      const response = await axios.post('http://localhost:2014/changejobstatus', { applicantId, status });
      fetchJobApplicants();
